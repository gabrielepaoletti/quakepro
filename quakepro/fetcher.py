...
$ seismic-downloader -p "INGV" -n "IV" -s "MMO1" -l "*" -c "EH*" -m "iasp91" --time_before_p 5 --time_after_p 15 --catalog_path "path_to_your_catalog.csv"
...
class WaveformFetcher:
    ...
    def _append_attributes(self, event: pd.Series, dataset: h5py.Dataset) -> None:
        ...
        event_attributes = {
                ...
                'receiver_name': self.station.name,
                'receiver_type': self.channel[0:2],
                'receiver_latitude': self.station.latitude,
                'receiver_longitude': self.station.longitude,
                'receiver_elevation_m': self.station.elevation,
                'receiver_sampling_rate_hz': self.station.sampling_rate,
                'source_id': event.id,
                'source_depth_km': event.depth,
                'source_latitude': event.lon,
                'source_longitude': event.lat,
                'source_magnitude': event.magnitude,
                'source_origin_time': event.time,
